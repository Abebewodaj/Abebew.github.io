...
let names:string[] =["James","Smith","Kal","Lemma","Kebede","Abebe"];

 let [Name1,Name2,...theRest]:string[] = names;
...
return totalPlayersPoint;
 ...
 function findTotalScores(players: Player[]): { jersey: number; total: number }[] {
  const result: { jersey: number; total: number }[] = [];
